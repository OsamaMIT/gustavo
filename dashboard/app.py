...
def build_dashboard_state(scenario: str) -> dict[str, Any]:
    ...
    engine = DiagnosisEngine(hypotheses, tests, likelihoods)
...
def main() -> None:
    ...
    source = st.sidebar.radio("Telemetry source", ["Synthetic scenario", "CSV upload"])
    ...
    if st.button("Run 25-trial validation"):
        result = compare_strategies(trials=25, objective=objective)
        st.dataframe(
            [
                {
                    "strategy": strategy,
